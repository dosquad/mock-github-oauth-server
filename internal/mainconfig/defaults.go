...
func ConfigInit() {
	viper.SetConfigName("mock-server")
	viper.SetConfigType("toml")
	viper.AddConfigPath("./artifacts")
	viper.AddConfigPath("./test")
	viper.AddConfigPath("$HOME/.mock-server")
	viper.AddConfigPath("$HOME/.config")
	viper.AddConfigPath("/run/secrets")
	viper.AddConfigPath("/etc/mock-server")
	viper.AddConfigPath("/usr/local/etc")
	viper.AddConfigPath("/usr/local/mock-server/etc")
	viper.AddConfigPath("/opt/homebrew/etc")
	viper.AddConfigPath("/etc/nsca")
	viper.AddConfigPath("/etc/nagios")
	viper.AddConfigPath(".")

	viper.SetDefault("server.bind", "localhost:8080")
	...
	// viper.SetDefault("server.state-store", "/tmp/mock-server-state.db")
